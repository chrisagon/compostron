...
<script>
	$j(function() {
		...
		var data = {
			...
			dans_le_bac: <?php echo json_encode(array('id' => $rdata['dans_le_bac'], 'value' => $rdata['dans_le_bac'], 'text' => $jdata['dans_le_bac'])); ?>
		};
		...
		/* saved value for dans_le_bac */
		cache.addCheck(function(u, d) {
			if(u != 'ajax_combo.php') return false;
			if(d.t == tn && d.f == 'dans_le_bac' && d.id == data.dans_le_bac.id)
				return { results: [ data.dans_le_bac ], more: false, elapsed: 0.01 };
			return false;
		});
