	...
	*/
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = array(
			...
			'depots' => array('D&#233;p&#244;ts', 'Indiquez les d&#233;p&#244;ts que vous avez effectu&#233;s sur votre site.', 'resources/table_icons/light_circle_green.png', 'Principale'),
			'site_compostage' => array('Site compostage', 'Site de d&#233;p&#244;t de vos mati&#232;res compostables et de retrait de votre compost.', 'resources/table_icons/google_map.png', 'Principale'),
			'bacs_du_site' => array('Bacs du site', 'Permet d\'inventorier les bacs de compostage de chaque site.', 'resources/table_icons/recycle_bag.png', 'Administration'),
			'membres' => array('Membres', 'Membres des sites du Composthon.', 'resources/table_icons/group.png', 'Administration'),
			'transferts' => array('Transferts', 'Les transferts entre bacs sont r&#233;alis&#233;s par le technicien.', 'resources/table_icons/arrow_switch.png', 'Technique'),
			'intervention' => array('Intervention', 'Demande d\'intervention sur site d\'un technicien.', 'resources/table_icons/alarm_bell.png', 'Principale')
		);
	...
	function get_table_groups($skip_authentication = false) {
		...
		$all_groups = array('Principale', 'Technique', 'Administration');
	...
	function get_sql_fields($table_name) {
		$sql_fields = array(
			'depots' => "`depots`.`id` as 'id', IF(    CHAR_LENGTH(`site_compostage1`.`nom_du_site`), CONCAT_WS('',   `site_compostage1`.`nom_du_site`, ' '), '') as 'sur_le_site', IF(    CHAR_LENGTH(`membres1`.`prenom`) || CHAR_LENGTH(`membres1`.`Nom`), CONCAT_WS('',   `membres1`.`prenom`, `membres1`.`Nom`), '') as 'par_le_membre', IF(    CHAR_LENGTH(`bacs_du_site1`.`type_de_bacs`) || CHAR_LENGTH(`site_compostage2`.`nom_du_site`), CONCAT_WS('',   `bacs_du_site1`.`type_de_bacs`, ' du site ', `site_compostage2`.`nom_du_site`), '') as 'dans_le_bac', DATE_FORMAT(`depots`.`jour_heure`, '%Y/%m/%d') as 'jour_heure', `depots`.`qte_vmh` as 'qte_vmh', `depots`.`qte_bds` as 'qte_bds', `depots`.`niveau_du_bac` as 'niveau_du_bac', `depots`.`cree_par` as 'cree_par', `depots`.`date_maj` as 'date_maj'",
			'site_compostage' => "`site_compostage`.`id` as 'id', `site_compostage`.`nom_du_site` as 'nom_du_site', `site_compostage`.`adresse_postale` as 'adresse_postale', `site_compostage`.`acces_au_site` as 'acces_au_site', `site_compostage`.`type_de_site` as 'type_de_site', `site_compostage`.`nom_du_responsable` as 'nom_du_responsable', `site_compostage`.`email_du_responsable` as 'email_du_responsable', `site_compostage`.`Telephone_du_responsable` as 'Telephone_du_responsable', `site_compostage`.`Commentaires` as 'Commentaires'",
			'bacs_du_site' => "`bacs_du_site`.`id` as 'id', IF(    CHAR_LENGTH(`site_compostage1`.`nom_du_site`), CONCAT_WS('',   `site_compostage1`.`nom_du_site`), '') as 'appartient_site', `bacs_du_site`.`type_de_bacs` as 'type_de_bacs', `bacs_du_site`.`niveau_de_remplissage` as 'niveau_de_remplissage', `bacs_du_site`.`Commentaires` as 'Commentaires'",
			'membres' => "`membres`.`id` as 'id', `membres`.`Nom` as 'Nom', `membres`.`prenom` as 'prenom', `membres`.`statut` as 'statut', `membres`.`adresse_du_membre` as 'adresse_du_membre', `membres`.`email_membre` as 'email_membre', `membres`.`Telephone_membre` as 'Telephone_membre'",
			'transferts' => "`transferts`.`id` as 'id', IF(    CHAR_LENGTH(`site_compostage1`.`nom_du_site`), CONCAT_WS('',   `site_compostage1`.`nom_du_site`), '') as 'sur_le_site', IF(    CHAR_LENGTH(`membres1`.`Nom`) || CHAR_LENGTH(`membres1`.`prenom`), CONCAT_WS('',   `membres1`.`Nom`, `membres1`.`prenom`), '') as 'par_le_membre', DATE_FORMAT(`transferts`.`jour_heure`, '%Y/%m/%d') as 'jour_heure', `transferts`.`du_bac` as 'du_bac', `transferts`.`vers_bac` as 'vers_bac', `transferts`.`quantite` as 'quantite', `transferts`.`cree_par` as 'cree_par', `transferts`.`date_maj` as 'date_maj'",
			'intervention' => "`intervention`.`id` as 'id', IF(    CHAR_LENGTH(`membres1`.`Nom`) || CHAR_LENGTH(`membres1`.`prenom`), CONCAT_WS('',   `membres1`.`Nom`, `membres1`.`prenom`), '') as 'demande_par', DATE_FORMAT(`intervention`.`date_demande`, '%Y/%m/%d') as 'date_demande', IF(    CHAR_LENGTH(`membres2`.`Nom`) || CHAR_LENGTH(`membres2`.`prenom`), CONCAT_WS('',   `membres2`.`Nom`, `membres2`.`prenom`), '') as 'realise_par', DATE_FORMAT(`intervention`.`date_realisation`, '%Y/%m/%d') as 'date_realisation', IF(    CHAR_LENGTH(`site_compostage1`.`nom_du_site`), CONCAT_WS('',   `site_compostage1`.`nom_du_site`), '') as 'pour_site', IF(    CHAR_LENGTH(`bacs_du_site1`.`type_de_bacs`) || CHAR_LENGTH(`site_compostage2`.`nom_du_site`), CONCAT_WS('',   `bacs_du_site1`.`type_de_bacs`, ' du site ', `site_compostage2`.`nom_du_site`), '') as 'pour_bac', `intervention`.`type_intervention` as 'type_intervention', `intervention`.`statut_intervention` as 'statut_intervention', `intervention`.`description` as 'description', `intervention`.`cree_par` as 'cree_par', `intervention`.`date_maj` as 'date_maj'",
		);
	...
	function get_defaults($table) {
		...
		$defaults = array(
			...
			'site_compostage' => array(
				...
				'type_de_site' => '',
				'nom_du_responsable' => '',
				'email_du_responsable' => 'adresse@email.com',
			...
			'intervention' => array(
				...
				'pour_site' => '',
				'pour_bac' => '',
				'type_intervention' => 'Transfert',
				'statut_intervention' => 'En attente',
				'description' => '',
				'cree_par' => '',
				'date_maj' => ''
			)
	...
	function htmlUserBar() {
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> compostron</a>
			</div>
